"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.deleteAll = void 0;
var app_1 = require("firebase/app");
var storage_1 = require("firebase/storage");
var firestore_1 = require("firebase-admin/firestore");
var firebase_1 = require("./firebase");
var validate_1 = require("./validate");
var admin = require("firebase-admin");
var deleteAll = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var packageName, rawHeaders, authHeaderIndex, token, authentication, firebaseApp, storage, storageRef, fileList, _i, _a, item, db, packagesRef, docs, idArray_2, _b, idArray_1, id, tempIdRef, storageFolder, err_1;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                packageName = req.params["packageName"];
                console.log("DeleteAll: packageName ".concat(packageName));
                rawHeaders = req.rawHeaders;
                authHeaderIndex = rawHeaders.indexOf("X-Authorization");
                token = authHeaderIndex !== -1 ? rawHeaders[authHeaderIndex + 1] : undefined;
                if (!(token && packageName)) return [3 /*break*/, 18];
                return [4 /*yield*/, (0, validate_1.validation)(token)];
            case 1:
                authentication = _c.sent();
                if (!authentication[0]) return [3 /*break*/, 16];
                _c.label = 2;
            case 2:
                _c.trys.push([2, 14, , 15]);
                firebaseApp = (0, app_1.initializeApp)(firebase_1.firebaseConfig);
                storage = (0, storage_1.getStorage)(firebaseApp);
                storageRef = (0, storage_1.ref)(storage, "".concat(packageName));
                return [4 /*yield*/, (0, storage_1.listAll)(storageRef)];
            case 3:
                fileList = _c.sent();
                _i = 0, _a = fileList.items;
                _c.label = 4;
            case 4:
                if (!(_i < _a.length)) return [3 /*break*/, 7];
                item = _a[_i];
                return [4 /*yield*/, (0, storage_1.deleteObject)((0, storage_1.ref)(storage, item.fullPath))];
            case 5:
                _c.sent();
                _c.label = 6;
            case 6:
                _i++;
                return [3 /*break*/, 4];
            case 7:
                console.log("DeleteAll: deleted from storage");
                db = (0, firestore_1.getFirestore)(admin.apps[0]);
                packagesRef = db.collection(packageName);
                return [4 /*yield*/, packagesRef.get()];
            case 8:
                docs = _c.sent();
                idArray_2 = [];
                docs.forEach(function (doc) {
                    var docData = doc.data();
                    if (docData["ID"] != undefined) {
                        idArray_2.push(docData["ID"]);
                    }
                    doc.ref["delete"]();
                });
                console.log("DeleteAll: deleted from firestore");
                _b = 0, idArray_1 = idArray_2;
                _c.label = 9;
            case 9:
                if (!(_b < idArray_1.length)) return [3 /*break*/, 12];
                id = idArray_1[_b];
                tempIdRef = db.collection("ID").doc(id);
                return [4 /*yield*/, tempIdRef["delete"]()];
            case 10:
                _c.sent();
                _c.label = 11;
            case 11:
                _b++;
                return [3 /*break*/, 9];
            case 12:
                storageFolder = db.collection("storage").doc(packageName);
                return [4 /*yield*/, storageFolder["delete"]()];
            case 13:
                _c.sent();
                res.status(200).send("Package is deleted");
                return [3 /*break*/, 15];
            case 14:
                err_1 = _c.sent();
                console.log(err_1);
                res.status(404).send("Package does not exist.");
                return [3 /*break*/, 15];
            case 15: return [3 /*break*/, 17];
            case 16:
                console.log("DeleteAll: Wrong authentication token");
                res.status(400).send("The AuthenticationToken is invalid.");
                _c.label = 17;
            case 17: return [3 /*break*/, 19];
            case 18:
                console.log("DeleteAll: Missing field(s)");
                res.status(400).send("There is missing field(s) in the PackageID/AuthenticationToken or it is formed improperly, or the AuthenticationToken is invalid.");
                _c.label = 19;
            case 19: return [2 /*return*/];
        }
    });
}); };
exports.deleteAll = deleteAll;
