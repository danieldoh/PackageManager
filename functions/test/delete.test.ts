...
describe("fileDelete", () => {
  ...
  it("(2) should return 404 if the package does not exist", async () => {
    ...
    const db = {
      collection: jest.fn().mockReturnThis(),
      doc: jest.fn().mockReturnThis(),
      delete: jest.fn().mockRejectedValueOnce(new Error("Document does not exist.")),
    };
    firestore.mockReturnValueOnce(db);
...
it("(5) should return 404 if package does not exist", async () => {
    ...
    const deleteObject = jest.fn().mockRejectedValueOnce(new Error("Object does not exist"));
    const storageRef = {
        child: jest.fn().mockReturnThis(),
        delete: deleteObject,
    };
